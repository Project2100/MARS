...
public class JEditBasedTextArea extends JEditTextArea implements MARSTextEditingArea, CaretListener {
    ...
    public JEditBasedTextArea(EditPane editPain, JComponent lineNumbers) {
        ...
        undoableEditListener = (UndoableEditEvent e) -> {
        ...
        setFont(FontSettings.EDITOR_FONT.get());
...
// 		 public void setSize(Dimension d) { painter.setSize(d);}
    ...
    public void undo() {
        ...
        try {
            ...
            Main.logger.log(Level.SEVERE, "Unable to undo", ex);
        }
        unredoing = false;
    ...
    public void redo() {
        ...
        try {
            ...
            Main.logger.log(Level.SEVERE, "Unable to redo", ex);
        }
        unredoing = false;
        this.setCaretVisible(true);
    }
    ...
    public int doFindText(String find, boolean caseSensitive) {
        int findPosn = getCaretPosition();
        int nextPosn;
        nextPosn = nextIndex(getText(), find, findPosn, caseSensitive);
        if (nextPosn >= 0) {
            requestFocus(); // guarantees visibility of the blue highlight 
            setSelectionStart(nextPosn); // position cursor at word start
            setSelectionEnd(nextPosn + find.length());
            // Need to repeat start due to quirk in JEditTextArea implementation of setSelectionStart.
            setSelectionStart(nextPosn);
            return TEXT_FOUND;
        }
        else
    ...
    public int doReplace(String find, String replace, boolean caseSensitive) {
        ...
        if (find == null || !find.equals(getSelectedText())
                || getSelectionEnd() != getCaretPosition())
            return doFindText(find, caseSensitive);
        // We are positioned at end of selected "find".  Rreplace and find next.
        nextPosn = getSelectionStart();
        grabFocus();
        setSelectionStart(nextPosn); // posn cursor at word start
        setSelectionEnd(nextPosn + find.length()); //select found text
        // Need to repeat start due to quirk in JEditTextArea implementation of setSelectionStart.
        setSelectionStart(nextPosn);
        isCompoundEdit = true;
        compoundEdit = new CompoundEdit();
        ...
        setCaretPosition(nextPosn + replace.length());
